  ...
  return (
    <main className="min-h-screen bg-neutral-50 dark:bg-neutral-950">
      ...
      <div className="mx-auto max-w-2xl px-4 py-6 sm:px-6 sm:py-8">
        {parent && (
          <a href={`/parents/${parent.id}`} className="text-sm text-neutral-400 underline dark:text-neutral-500">
        ...
        {reservation && (
          <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
            ...
            <a href={`/reservations/${reservation.id}`} className="ml-1 text-xs underline text-neutral-400 dark:text-neutral-500">
              View reservation →
            </a>
          </p>
        )}
        ...
        <div className="mt-6 rounded-lg border border-neutral-200 bg-white p-4 sm:p-6 dark:border-neutral-800 dark:bg-neutral-900">
          <table className="w-full text-sm">
            ...
            <tbody>
              {(lineItems ?? []).map((li) => (
                <tr key={li.id} className="border-b border-neutral-100 last:border-0 dark:border-neutral-800">
          ...
          <div className="mt-4 flex flex-col items-end gap-1 border-t border-neutral-100 pt-4 text-sm dark:border-neutral-800">
            <div className="flex w-40 justify-between">
              <span className="text-neutral-400 dark:text-neutral-500">Subtotal</span>
              <span>{money(Number(invoice.subtotal))}</span>
            </div>
            <div className="flex w-40 justify-between">
              <span className="text-neutral-400 dark:text-neutral-500">Tax</span>
