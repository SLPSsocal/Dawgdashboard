      ...
      <div className="mx-auto max-w-2xl px-4 py-6 sm:px-6 sm:py-8">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <a
            ...
            target="_blank"
            className="rounded-lg border border-neutral-300 px-3 py-1.5 text-xs font-medium hover:border-neutral-500 dark:border-neutral-700 dark:hover:border-neutral-500"
          >
            🖨️ Print Run Card
          </a>
          {animal && (
          ...
          <form action={updateWithId} className="flex flex-col gap-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <label className="block">
                <span className="text-sm font-medium text-neutral-700 dark:text-neutral-300">Arrival</span>
                <input
                  name="start_date"
                  type="datetime-local"
              ...
              <label className="block">
                <span className="text-sm font-medium text-neutral-700 dark:text-neutral-300">Departure</span>
                <input
                  name="end_date"
                  type="datetime-local"
            ...
            <label className="block">
              <span className="text-sm font-medium text-neutral-700 dark:text-neutral-300">Reservation Type</span>
              <select
            ...
            <label className="block">
              <span className="text-sm font-medium text-neutral-700 dark:text-neutral-300">Lodging</span>
              <select
            ...
            <label className="block">
              <span className="text-sm font-medium text-neutral-700 dark:text-neutral-300">Belongings</span>
              <input
                name="belongings"
        ...
        {(incidents?.length ?? 0) > 0 && (
          <div className="mt-4 rounded-lg border border-neutral-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-900">
            <h2 className="text-sm font-semibold text-neutral-700 dark:text-neutral-200">⚠️ Incidents</h2>
            <div className="mt-2 flex flex-col gap-2">
              {incidents!.map((inc) => (
                <div key={inc.id} className="rounded-md border border-neutral-200 px-3 py-2 text-sm dark:border-neutral-800">
                  <div className="font-medium capitalize">{inc.severity}</div>
                  <div className="text-neutral-500 dark:text-neutral-400">{inc.description}</div>
        ...
        {(reportCards?.length ?? 0) > 0 && (
          <div className="mt-4 rounded-lg border border-neutral-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-900">
            <h2 className="text-sm font-semibold text-neutral-700 dark:text-neutral-200">❤️ Report Cards</h2>
            <div className="mt-2 flex flex-col gap-2">
              {reportCards!.map((rc) => (
                <div key={rc.id} className="rounded-md border border-neutral-200 px-3 py-2 text-sm dark:border-neutral-800">
                  <div className="font-medium capitalize">{rc.rating ?? "—"}</div>
                  {rc.activities && rc.activities.length > 0 && (
                    <div className="text-neutral-500 dark:text-neutral-400">{rc.activities.join(", ")}</div>
                  )}
                  {rc.notes && <div className="text-neutral-500 dark:text-neutral-400">{rc.notes}</div>}
                  <div className="mt-1 text-xs text-neutral-400 dark:text-neutral-500">
