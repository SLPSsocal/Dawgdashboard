...
// Receipts tied to this dog specifically — invoices whose reservation
// belongs to this animal_id. (A walk-in retail sale on the parent's account
// isn't tied to any one dog, so it shows on the parent's invoice history
// instead, not here — this page is about what this dog's stays cost.)
export default async function AnimalInvoiceHistoryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: resRows } = await supabase.from("reservations").select("id").eq("animal_id", id);
  const reservationIds = (resRows ?? []).map((r) => r.id);

  const { data: invoiceData } = reservationIds.length
    ? await supabase
        .from("invoices")
