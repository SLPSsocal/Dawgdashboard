  ...
  function Card({ c, draggable }: { c: ApptCard; draggable: boolean }) {
    return (
      <div
        ...
        className={`rounded-md border bg-white px-3 py-2 text-sm shadow-sm dark:bg-neutral-900 ${
      ...
      >
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="shrink-0 text-xs text-neutral-400 dark:text-neutral-500">{fmtTime(c.time)}</span>
        </div>
    ...
    return (
      ...
      >
        <div className="text-sm font-semibold">
          {name} <span className="font-normal text-neutral-400 dark:text-neutral-500">({items.length})</span>
        </div>
        <div className="mt-2 flex flex-col gap-2">
          {items.map((c) => (
            <Card key={c.id} c={c} draggable />
          ))}
          {items.length === 0 && <div className="text-xs text-neutral-300 dark:text-neutral-600">Empty</div>}
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={`min-h-[110px] rounded-xl border p-3 ${accent}`}>
        ...
        <div className="mt-2 flex flex-col gap-2">
          ...
          {items.length === 0 && <div className="text-xs text-neutral-300 dark:text-neutral-600">Nothing today</div>}
        </div>
      </div>
    );
  }
    ...
    <div className="mt-3">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <InfoColumn
          ...
          items={incoming}
          accent="border-neutral-300 bg-neutral-50 dark:border-neutral-700 dark:bg-neutral-900/40"
        />
      </div>
    </div>
  );
